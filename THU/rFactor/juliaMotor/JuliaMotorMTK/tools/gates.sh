...
PROJ="$(cd "$HERE/.." && pwd)"
FILTER="${1:-}"
SMOKES="parse_smoke wreck_smoke contact_smoke stacked_contact_smoke boundary3d_smoke extforce3d_smoke wheelmu_smoke drive3d_smoke stall_smoke transmission_smoke controls_smoke people_smoke damage_smoke mipcolor_smoke ai_field_smoke susp_pose_smoke netplay_smoke setup_tab_smoke offroad_smoke wreck_seal_smoke reground_smoke netplay_dr_smoke netplay_dr2_smoke hat_hole_smoke clutchgate_smoke contact_geom_smoke lapprog_smoke restart_smoke softband_smoke vtbrake_smoke netai_smoke racestart_smoke step_guard_smoke"
...
for g in $SMOKES; do
  ...
  gproj="$PROJ"; case "$g" in susp_pose_smoke|netplay_smoke|setup_tab_smoke|reground_smoke|netplay_dr_smoke|netplay_dr2_smoke) gproj="$PROJ/../demo/native" ;; esac
