...
| E72 | **Watkins Glen** | 5 (S1–S5) | both censuses CLEAN (nearest crowd 13.2 m, 0 footprints crossing); PO crowd block not reproduced; S2: CLEAN on all three censuses at the corrected 5.05 m edge (0 footprints, 16/16 alignment, crowds ≥13.2 m); S3: dedup is NOT broken — only 1 duplicate exists in 2436 rail tris (per-part = global), so the 13% premise does not reproduce; W7 needs re-scoping away from dedup. S4: pit structures ARE placed+kept (not missing) at lat −28..−42. S5: and NOT VISIBLE at their own lapdists — tower1 at 26.9 m should clip the frame and does not; next = check base HEIGHT vs terrain (buried?) — see e72_watkins.md |
