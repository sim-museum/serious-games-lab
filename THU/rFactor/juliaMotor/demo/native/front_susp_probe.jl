...
tor(v) = (Float64(v[1]), Float64(v[3]), Float64(v[2]))   # x fwd, y up, z lateral
acc = Dict{Tuple{String,Int},Any}()
for (k,t) in enumerate(m.tris)
    t.tex in ("lsusp1","frontlot","front1","front3","lobrali") || continue
    a = get!(acc, (t.tex, m.groups[k]), Any[0, fill(Inf,3), fill(-Inf,3), 0.0])
    a[1] += 1
    for v in t.p; r = tor(v); for i in 1:3; a[2][i]=min(a[2][i],r[i]); a[3][i]=max(a[3][i],r[i]); end; end
    e(a,b) = sqrt(sum((a[i]-b[i])^2 for i in 1:3)); a[4] = max(a[4], e(t.p[1],t.p[2]), e(t.p[2],t.p[3]), e(t.p[1],t.p[3]))
end
println("front-end parts per (texture, group), car space; front axle at x=+1.31, front hubs z=+-0.75:")
