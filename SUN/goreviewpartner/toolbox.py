...
def convert_sgf_to_utf(content):
	...
	if node_has(gameroot,"CA"):
		...
		if ca=="UTF-8":
			...
			game = sgf.Sgf_game.from_string(content) #sgf.Sgf_game.from_string requires str object, not unicode
			return game
	else:
...
class RangeSelector(Toplevel):
	...
	def start(self):
		...
		if self.mode.get()=="all":
			intervals="all moves"
			move_selection=list(range(1,self.nb_moves+1))
		else:
...
class RunAnalysisBase(Toplevel):
	...
	def initialize_UI(self):
		...
		if not self.move_range:
			self.move_range=list(range(1,self.max_move+1))
...
def parse_command_line(filename,argv):
	...
	#argv=[(str(p,errors="replace"),str(v,errors="replace")) for p,v in argv] #ok, this is maybe overkill...
	for p,v in argv:
		if p=="--variation":
			try:
				variation=int(v)
	...
	if not found:
		move_selection=list(range(1,nb_moves+1))
		intervals="all moves"
		log("Range: all")
...
try:
	conf.read_file(codecs.open(config_file,"r","utf-8"))
except Exception as e:
...
def node_set(node, property_name, value):
	...
	if property_name.lower() in ("w","b"):
		node.set_move(property_name,value)
	elif  property_name.upper() in ("TBM","TWM", "IBM", "IWM"):
		new_list=[]
		for ij in value:
			new_list.append(ij2sgf(ij))
