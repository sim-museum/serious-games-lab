...
void CheckForClimb(WayPoint cw)
{
    ...
    if (lw)
    {
        ...
        if (alt > 0 and abs(altd) > GRID_SIZE_FT)
        {
            GridIndex x, y, lx, ly;
            int maxdelta;
...
void FinalizeFillerWayPoint(WayPoint cw)
{
    ...
    if (lw)
    {
        ...
        if (sCurrentAlt > 0 and abs(altd) > GRID_SIZE_FT)
        {
            // Just climb at max
            GridIndex x, y, lx, ly;
            int maxdelta;
...
// Adds a land and takeoff WP at target
WayPoint AddLandProfile(WayPoint cw, Flight u, MissionRequestClass *mis)
{
    WayPoint tw;
    ...
    // tw = new WayPointClass(mis->tx, mis->ty, 0, 0, 0, 0, WP_TAKEOFF, WPF_TARGET bitor WPF_TAKEOFF);
    // FinalizeWayPoint(tw);
    // cw->InsertWP(tw);
    return tw;
}
...
long SetWPTimes(Flight u, MissionRequestClass *mis)
{
    ...
    while (w)
    {
        ...
        if (w->GetWPFlags() bitand WPF_TARGET and not tw)
        {
            tw = w;
            w->SetWPTimes(mis->tot);
        }

        if (w->GetWPFlags() bitand WPF_TURNPOINT and not tw)
            tw = w;
...
int ScoreThreatsOnWPLeg(WayPoint w1, WayPoint w2, Team who, int type)
{
    ...
    for (step = 0; step <= dist; step += MAP_RATIO)
    {
        ...
        if (type == TT_MAX and threat > worst)
            worst = threat;
        else if (type == TT_TOTAL or type == TT_AVERAGE)
            worst += threat;
...
int CheckBestAltitude(GridIndex tx, GridIndex ty, Team who, int min, int max, int try_for, int type)
{
    ...
    for (a = 1; a < ALT_LEVELS; a++)
    {
        ...
        if (MinAltAtLevel[a] > max and ha == ALT_LEVELS)
            ha = a;
    }
    ...
    for (d = 0; d <= Here; d += 2)
    {
        ...
        for (a = la; a < ha; a++)
        {
            ...
            if (type == TT_TOTAL or type == TT_AVERAGE)
                score[a] += threat;
            else if (type == TT_MAX and threat > score[a])
                score[a] = threat;
        }
    }
...
WayPoint FillAirPath(Path path, GridIndex *x, GridIndex *y, GridIndex nx, GridIndex ny, WayPoint w)
{
    ...
    if (QuickSearch and DistSqu(*x, *y, nx, ny) < QuickSearch * QuickSearch)
    {
        *x = nx;
        *y = ny;
    }
...
WayPoint EliminateExcessWaypoints(WayPoint w1, WayPoint w2, int who)
{
    ...
    while (w and mw and nw and w not_eq w2 and mw not_eq w2)
    {
        ...
        if (mw->GetWPAction() == WP_NOTHING and not (mw->GetWPFlags() bitand WPF_CRITICAL_MASK))
        {
            ...
            if (cwm > COS_10 or cwm < COS_120 or cnm > COS_10 or cnm < COS_120)
            {
#ifdef DEBUG
                trimmed_by_angle++;
#endif
...
WayPoint AddSafeWaypoint(WayPoint w1, WayPoint w2, int type, int distance, Team who)
{
    ...
    while (w and w not_eq w2 and not done)
    {
        if (type)
            nw = w->GetPrevWP();
        else
        ...
        for (step = 0; step <= dist and not done; step += 10)
        {
    ...
    while (w and nw)
    {
        ...
        for (step = 0; step < dist; step += 5)
        {
            ...
            /* // This was only usefull when we wanted to actually target things which got in our way.
             if (CollectThreatsFast (x,y,GetAltitudeLevel(nw->GetWPAltitude()),u->GetTeam(),FIND_NOAIR bitor FIND_NOMOVERS bitor FIND_FINDUNSPOTTED,&threats))
             {
...
BOOL LoadMissionData()
{
    ...
    while (fgets(buffer, sizeof buffer, fp) not_eq NULL)
    {
        ...
        if (no < 0 or no >= sizeof(MissionData) / sizeof(MissionData[0]))
            continue;
