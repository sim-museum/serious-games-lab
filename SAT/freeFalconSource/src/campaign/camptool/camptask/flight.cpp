...
FlightClass::FlightClass(VU_BYTE **stream) : AirUnitClass(stream)
{
    ...
    memcpy(&pos_.z_, *stream, sizeof(BIG_SCALAR));
    *stream += sizeof(BIG_SCALAR);
    memcpy(&fuel_burnt, *stream, sizeof(long));
    *stream += sizeof(long);

    if (gCampDataVersion < 65)
        fuel_burnt = 0;

    memcpy(&last_move, *stream, sizeof(CampaignTime));
    *stream += sizeof(CampaignTime);
    memcpy(&last_combat, *stream, sizeof(CampaignTime));
    *stream += sizeof(CampaignTime);
    memcpy(&time_on_target, *stream, sizeof(CampaignTime));
    *stream += sizeof(CampaignTime);
    memcpy(&mission_over_time, *stream, sizeof(CampaignTime));
    *stream += sizeof(CampaignTime);
    memcpy(&mission_target, *stream, sizeof(short));
    *stream += sizeof(short);
    ...
    if (gCampDataVersion < 24)
    {
        ...
        {
            use_loadout = (char) **stream;
            *stream += sizeof(use_loadout);

            if (use_loadout)
            {
                LoadoutArray junk;
                memcpy(&junk, *stream, sizeof(LoadoutArray));
                *stream += sizeof(LoadoutArray);
        ...
        if (gCampDataVersion < 18)
        {
            ...
            memcpy(weapon, *stream, sizeof(short)*HARDPOINT_MAX);
            *stream += sizeof(short) * HARDPOINT_MAX;
        ...
        else
        {
            ...
            memcpy(weapon, *stream, sizeof(uchar)*HARDPOINT_MAX);
            *stream += sizeof(uchar) * HARDPOINT_MAX;

            if (!use_loadout)
                memcpy(loadout[0].WeaponID, weapon, sizeof(short)*HARDPOINT_MAX);
        }

        memcpy(weapons, *stream, sizeof(uchar)*HARDPOINT_MAX);
        *stream += sizeof(uchar) * HARDPOINT_MAX;
    ...
    {
        memcpy(&loadouts, *stream, sizeof(uchar));
        *stream += sizeof(uchar);
        loadout = new LoadoutStruct[loadouts];
        ...
        for (int i = 0; i < loadouts; i++)
        {
            ...
            {
                memcpy(&loadout[i], *stream, sizeof(LoadoutStruct));
                *stream += sizeof(LoadoutStruct);
            }
        }
    }

    memcpy(&mission, *stream, sizeof(uchar));
    *stream += sizeof(uchar);

    if (gCampDataVersion > 65)
    {
        memcpy(&old_mission, *stream, sizeof(uchar));
        *stream += sizeof(uchar);
    }
    else
        old_mission = mission;

    memcpy(&last_direction, *stream, sizeof(uchar));
    *stream += sizeof(uchar);
    memcpy(&priority, *stream, sizeof(uchar));
    *stream += sizeof(uchar);
    memcpy(&mission_id, *stream, sizeof(uchar));
    *stream += sizeof(uchar);
    ...
    memcpy(&eval_flags, *stream , sizeof(uchar));
    *stream += sizeof(uchar);

    if (gCampDataVersion > 65)
    {
        memcpy(&mission_context, *stream , sizeof(uchar));
        *stream += sizeof(uchar);
    }
    else
        mission_context = 0;

    memcpy(&package, *stream, sizeof(VU_ID));
    *stream += sizeof(VU_ID);
    ...
    memcpy(&squadron, *stream, sizeof(VU_ID));
    *stream += sizeof(VU_ID);

    if (gCampDataVersion > 65)
    {
        memcpy(&requester, *stream, sizeof(VU_ID));
        *stream += sizeof(VU_ID);
    }
    else
        requester = FalconNullId;
...
#endif
    memcpy(slots, *stream, sizeof(uchar)*PILOTS_PER_FLIGHT);
    *stream += sizeof(uchar) * PILOTS_PER_FLIGHT;
    memcpy(pilots, *stream, sizeof(uchar)*PILOTS_PER_FLIGHT);
    *stream += sizeof(uchar) * PILOTS_PER_FLIGHT;
    memcpy(plane_stats, *stream, sizeof(uchar)*PILOTS_PER_FLIGHT);
    *stream += sizeof(uchar) * PILOTS_PER_FLIGHT;
    memcpy(player_slots, *stream, sizeof(uchar)*PILOTS_PER_FLIGHT);
    *stream += sizeof(uchar) * PILOTS_PER_FLIGHT;
    memcpy(&last_player_slot, *stream, sizeof(uchar));
    *stream += sizeof(uchar);
    memcpy(&callsign_id, *stream, sizeof(uchar));
    *stream += sizeof(uchar);
    memcpy(&callsign_num, *stream, sizeof(uchar));
    *stream += sizeof(uchar);

    if (gCampDataVersion >= 72)
    {

        memcpy(&refuel, *stream, sizeof(unsigned int));
