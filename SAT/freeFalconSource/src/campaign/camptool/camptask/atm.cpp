...
{
    memcpy(&id, *stream, sizeof(VU_ID));
    *stream += sizeof(VU_ID);
#ifdef DEBUG
    id.num_ &= 0xffff;
#endif
    memcpy(schedule, *stream, ATM_MAX_CYCLES);
...
AirTaskingManagerClass::AirTaskingManagerClass(VU_BYTE **stream) : CampManagerClass(stream)
{
    ...
    *stream += sizeof(short);
    memcpy(&averageCAStrength, *stream, sizeof(short));
    *stream += sizeof(short);
    memcpy(&averageCAMissions, *stream, sizeof(short));
    *stream += sizeof(short);
    memcpy(&sampleCycles, *stream, sizeof(uchar));
    *stream += sizeof(uchar);
    currentCAMissions = 0;
    memcpy(&num, *stream, sizeof(uchar));
    *stream += sizeof(uchar);
    airbaseList = NULL;
    ...
    memcpy(&cycle, *stream, sizeof(uchar));
    *stream += sizeof(uchar);
