...
SquadronClass::SquadronClass(VU_BYTE **stream) : AirUnitClass(stream)
{
    ...
    memcpy(&fuel, *stream, sizeof(long));
    *stream += sizeof(long);
    memcpy(&specialty, *stream, sizeof(uchar));
    *stream += sizeof(uchar);

    if (gCampDataVersion < 69)
    {
        memset(stores, 0, MAXIMUM_WEAPTYPES);
        memcpy(stores, *stream, sizeof(uchar) * 200);
        *stream += sizeof(uchar) * 200;
    }
    else if (gCampDataVersion < 72)
    {
        memset(stores, 0, MAXIMUM_WEAPTYPES);
        memcpy(stores, *stream, sizeof(uchar) * 220);
        *stream += sizeof(uchar) * 220;
    }
    else
    {
        memcpy(stores, *stream, sizeof(uchar)*MAXIMUM_WEAPTYPES);
    ...
    if (gCampDataVersion < 47)
    {
        ...
        {
            memcpy(pilot_data, *stream, 8 * PILOTS_PER_SQUADRON);
            *stream += 8 * PILOTS_PER_SQUADRON;
            // Reinit them
            InitPilots();
        }
        else
        {
            memcpy(pilot_data, *stream, 8 * 36);
            *stream += 8 * 36;
            // Reinit them
            InitPilots();
        }
    ...
    {
        memcpy(pilot_data, *stream, sizeof(PilotClass)*PILOTS_PER_SQUADRON);
    ...
    memcpy(schedule, *stream, sizeof(long)*VEHICLES_PER_UNIT);
    *stream += sizeof(long) * VEHICLES_PER_UNIT;
    memcpy(&airbase_id, *stream, sizeof(VU_ID));
    *stream += sizeof(VU_ID);
    ...
    *stream += sizeof(VU_ID);
    ...
    memcpy(rating, *stream, sizeof(uchar)*ARO_OTHER);
    *stream += sizeof(uchar) * ARO_OTHER;
    memcpy(&aa_kills, *stream, sizeof(short));
    *stream += sizeof(short);
    memcpy(&ag_kills, *stream, sizeof(short));
    *stream += sizeof(short);
    memcpy(&as_kills, *stream, sizeof(short));
    *stream += sizeof(short);
    memcpy(&an_kills, *stream, sizeof(short));
    *stream += sizeof(short);
    memcpy(&missions_flown, *stream, sizeof(short));
    *stream += sizeof(short);
    memcpy(&mission_score, *stream, sizeof(short));
    *stream += sizeof(short);
    memcpy(&total_losses, *stream, sizeof(uchar));
    *stream += sizeof(uchar);

    if (gCampDataVersion >= 9)
    {
        memcpy(&pilot_losses, *stream, sizeof(uchar));
        *stream += sizeof(uchar);
    }
    else
        pilot_losses = 0;
    ...
    {
        memcpy(&squadron_patch, *stream, sizeof(uchar));
        *stream += sizeof(uchar);
...
int SquadronClass::Save(VU_BYTE **stream)
{
    ...
    *stream += sizeof(PilotClass) * PILOTS_PER_SQUADRON;
    memcpy(*stream, schedule, sizeof(long)*VEHICLES_PER_UNIT);
    *stream += sizeof(long) * VEHICLES_PER_UNIT;
#ifdef CAMPTOOL
