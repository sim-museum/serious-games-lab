...
public:
    ...
    {
        ShiAssert(size == sizeof(DrawableTrail));
    ...
    {
        pool = MemPoolInitFS(sizeof(DrawableTrail), 40, 0);
    };
...
public:
    ...
    void *operator new(size_t size)
    {
        ShiAssert(size == sizeof(TrailElement));
        return MemAllocFS(pool);
    };
    void operator delete(void *mem)
    {
    ...
    static void InitializeStorage()
    {
        pool = MemPoolInitFS(sizeof(TrailElement), 100, 0);
    };
    static void ReleaseStorage()
    {
        MemPoolFree(pool);
...
#endif
public:
    TrailElement(Tpoint *p, DWORD now)
    {
        point = *p;
        time = now;
    };
