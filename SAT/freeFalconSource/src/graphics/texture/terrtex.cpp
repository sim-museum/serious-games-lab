...
void TextureDB::Load(SetEntry* pSet, TileEntry* pTile, int res, bool forceNoDDS)
{
    ...
    // Only accept DDS data if it has DXT compression flags (DXT1/DXT3/DXT5).
    // DDS files without DXT flags contain raw palette indices which require
    // a palette from the PCX file, so fall back to PCX loading for those.
    ...
    if ( not forceNoDDS and DisplayOptions.m_texMode == DisplayOptionsClass::TEX_MODE_DDS)
    {
        ...
        if (pTile->bits[res]) {
            bool hasDXT = (pTile->height[res] & (MPR_TI_DXT1 | MPR_TI_DXT3 | MPR_TI_DXT5)) != 0;
            if (hasDXT) {
                ddsLoaded = true;
                // Don't clear pSet->palette here - other resolution levels
                // in the same set may use PCX palette loading. DDS tiles
                // don't need the palette, but clearing it here would prevent
                // palette-based tiles (e.g., lower-res fallback) from rendering.
            } else {
                // Not real DXT data - free it and fall back to PCX
