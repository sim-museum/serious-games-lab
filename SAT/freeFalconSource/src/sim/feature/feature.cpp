...
int SimFeatureClass::Wake(void)
{
    ...
    VuListIterator elementWalker(GetCampaignObject()->GetComponents());
    ...
    if (drawPointer)
    {
        ...
        if (IsSetCampaignFlag(FEAT_HAS_SMOKE_STACK))
        {
            ...
            for (i = 0; i < num; i++)
            {
                // TODO:  Shouldn't this stay tied to the feature so it lives until reaggrigation???
                OTWDriver.AddSfxRequest(
                    new SfxClass(SFX_SMOKING_FEATURE, // type
                                 i, // slot #
                                 this, // world pos
                                 2.0f, // time to live
                                 40.0f)); // scale
