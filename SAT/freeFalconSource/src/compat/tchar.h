...
/* _tcsspnp - Returns pointer to first char not in charset, or NULL if all chars match */
static inline char* ff_strspnp(const char* str, const char* charset) {
    if (!str || !charset) return NULL;
    size_t span = strspn(str, charset);
    if (str[span] == '\0') return NULL;
