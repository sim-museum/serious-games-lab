...
#define strlwr          ff_strlwr
#define _strupr         ff_strupr
#define strupr          ff_strupr
...
static inline char* ff_ltoa(long value, char* str, int base) {
    if (base == 10) {
...
/* MulDiv - Windows API function for multiply then divide with 64-bit intermediate */
static inline int MulDiv(int nNumber, int nNumerator, int nDenominator) {
    if (nDenominator == 0) return -1;
    return (int)(((long long)nNumber * (long long)nNumerator) / (long long)nDenominator);
}
...
/* Math functions - Windows-specific */
#define _isnan      isnan
#define _isinf      isinf
...
/* Float to string conversion - MSVC specific */
/* _fcvt converts a float to a string. On Linux, use fcvt or sprintf */
static inline char* ff_fcvt(double value, int ndigit, int* decpt, int* sign) {
    ...
    *sign = (value < 0) ? 1 : 0;
    ...
    if (ndigit > 50) ndigit = 50;

    sprintf(buffer, "%.*f", ndigit, value);

    /* Find decimal point position */
    char* dot = strchr(buffer, '.');
    if (dot) {
        *decpt = (int)(dot - buffer);
        /* Remove the decimal point */
        memmove(dot, dot + 1, strlen(dot));
    } else {
        *decpt = (int)strlen(buffer);
    }

    return buffer;
}
#define _fcvt       ff_fcvt
