...
static inline BOOL PostMessageA(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam) {
    (void)hWnd;
    /* Route FM_* messages (WM_USER+1 and above) to the game message queue */
    if (Msg > WM_USER) {
...
static inline HWND FindWindowA(LPCSTR lpClassName, LPCSTR lpWindowName) {
    ...
    return NULL; /* Not found - can't enumerate windows on Linux */
}
...
/* FF_LINUX: ShowCursor must track display count like Windows.
 * Windows maintains an internal counter: count >= 0 means visible, count < 0 means hidden.
 * ShowCursor(TRUE) increments and returns new count.
 * ShowCursor(FALSE) decrements and returns new count.
 * Code like `while (ShowCursor(FALSE) >= 0);` depends on this behavior to hide cursor. */
static inline int ShowCursor(BOOL bShow) {
    static int cursor_display_count = 0;
    if (bShow) {
        cursor_display_count++;
    } else {
        cursor_display_count--;
    }
    return cursor_display_count;
}
...
/* Dialog functions */
static inline HWND GetDlgItem(HWND hDlg, int nIDDlgItem) {
    (void)hDlg; (void)nIDDlgItem;
    return NULL;
}

static inline UINT GetDlgItemTextA(HWND hDlg, int nIDDlgItem, LPSTR lpString, int cchMax) {
    (void)hDlg; (void)nIDDlgItem; (void)cchMax;
    if (lpString) lpString[0] = '\0';
    return 0;
}

static inline BOOL SetDlgItemTextA(HWND hDlg, int nIDDlgItem, LPCSTR lpString) {
    (void)hDlg; (void)nIDDlgItem; (void)lpString;
    return TRUE;
}

static inline BOOL CheckDlgButton(HWND hDlg, int nIDButton, UINT uCheck) {
    (void)hDlg; (void)nIDButton; (void)uCheck;
    return TRUE;
}

static inline UINT IsDlgButtonChecked(HWND hDlg, int nIDButton) {
    (void)hDlg; (void)nIDButton;
    return BST_UNCHECKED;
}

static inline BOOL EndDialog(HWND hDlg, INT_PTR nResult) {
    (void)hDlg; (void)nResult;
    return TRUE;
}

static inline int GetWindowTextA(HWND hWnd, LPSTR lpString, int nMaxCount) {
    (void)hWnd; (void)nMaxCount;
    if (lpString) lpString[0] = '\0';
    return 0;
}
...
static inline int GetWindowTextLengthA(HWND hWnd) {
    (void)hWnd;
    return 0;
}
...
static inline HMENU GetMenu(HWND hWnd) { (void)hWnd; return NULL; }
static inline HMENU GetSubMenu(HMENU hMenu, int nPos) { (void)hMenu; (void)nPos; return NULL; }
static inline BOOL EnableMenuItem(HMENU hMenu, UINT uIDEnableItem, UINT uEnable) {
    (void)hMenu; (void)uIDEnableItem; (void)uEnable; return TRUE;
}
static inline DWORD CheckMenuItem(HMENU hMenu, UINT uIDCheckItem, UINT uCheck) {
    (void)hMenu; (void)uIDCheckItem; (void)uCheck; return MF_UNCHECKED;
}
static inline BOOL SetMenu(HWND hWnd, HMENU hMenu) { (void)hWnd; (void)hMenu; return TRUE; }
static inline BOOL DrawMenuBar(HWND hWnd) { (void)hWnd; return TRUE; }
...
static inline HANDLE GetStdHandle(DWORD nStdHandle) { (void)nStdHandle; return NULL; }
static inline BOOL WriteConsoleA(HANDLE hConsoleOutput, const void *lpBuffer, DWORD nNumberOfCharsToWrite,
                                  LPDWORD lpNumberOfCharsWritten, LPVOID lpReserved) {
    (void)hConsoleOutput; (void)lpReserved;
    if (lpBuffer && nNumberOfCharsToWrite > 0) {
        fwrite(lpBuffer, 1, nNumberOfCharsToWrite, stdout);
    }
...
static inline BOOL SystemParametersInfoA(UINT uiAction, UINT uiParam, PVOID pvParam, UINT fWinIni) {
    (void)uiAction; (void)uiParam; (void)pvParam; (void)fWinIni;
    return TRUE; /* Stub - do nothing on Linux */
}
#define SystemParametersInfo SystemParametersInfoA

/* MessageBeep */
static inline BOOL MessageBeep(UINT uType) { (void)uType; return TRUE; }

/* ============================================================
 * Keyboard Functions
 * ============================================================ */

/* Virtual key scan - translates character to virtual key code */
static inline SHORT VkKeyScanA(CHAR ch) {
    /* Simple mapping for ASCII printable characters */
    if (ch >= 'a' && ch <= 'z') {
        return (SHORT)(ch - 'a' + 'A'); /* Return uppercase VK */
    }
    if (ch >= 'A' && ch <= 'Z') {
        return (SHORT)ch;
    }
    ...
    /* For other characters, return the character code */
    return (SHORT)(unsigned char)ch;
}
#define VkKeyScan VkKeyScanA

/* MapVirtualKey - maps virtual key codes, scan codes, and character values */
#define MAPVK_VK_TO_VSC     0
#define MAPVK_VSC_TO_VK     1
#define MAPVK_VK_TO_CHAR    2
#define MAPVK_VSC_TO_VK_EX  3

static inline UINT MapVirtualKeyA(UINT uCode, UINT uMapType) {
    (void)uMapType;
    /* Simple stub - return the code as-is for character mapping */
...
static inline HACCEL LoadAcceleratorsA(HINSTANCE hInstance, LPCSTR lpTableName) {
    (void)hInstance; (void)lpTableName;
    return NULL; /* No accelerator support on Linux */
}
#define LoadAccelerators LoadAcceleratorsA

static inline int TranslateAcceleratorA(HWND hWnd, HACCEL hAccTable, LPMSG lpMsg) {
    (void)hWnd; (void)hAccTable; (void)lpMsg;
    return 0; /* No accelerator support */
}
#define TranslateAccelerator TranslateAcceleratorA

static inline BOOL DestroyAcceleratorTable(HACCEL hAccel) {
    (void)hAccel;
    return TRUE;
}
