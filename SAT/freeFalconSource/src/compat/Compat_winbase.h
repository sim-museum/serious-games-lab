    ...
    LPDWORD lpThreadId)
{
    ...
    if (pthread_create(thread, NULL, ff_thread_start_routine, data) != 0) {
        free(thread);
        free(data);
        return NULL;
    }
    ...
    if (lpThreadId) {
        *lpThreadId = (DWORD)(intptr_t)*thread;
    }
...
static inline DWORD WaitForSingleObject(HANDLE hHandle, DWORD dwMilliseconds) {
    /* This is simplified - real implementation would need to know handle type */
...
/* Event structure (embedded in handle allocation) */
typedef struct {
