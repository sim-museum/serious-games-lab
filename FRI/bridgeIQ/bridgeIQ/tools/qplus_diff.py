"""Diff harness: bridgeIQ vs Q-Plus auctions.
...
Workflow:
  ...
  3. The script polls LOG/ until N new BDLs appear, then diffs
     each auction against bridgeIQ's bidder.
...
from backend.bdl_reader import load_bdl_file              # noqa: E402
...
def _read_deal_labels(bdl_path: Path) -> List[str]:
    ...
    Q-Plus's auctions back to bridgeIQ's board numbering.
...
def _read_qplus_system_from_bdl(bdl_path: Path) -> str:
    ...
    name in bridgeIQ's catalog so the diff compares like-with-
...
def main(argv=None) -> int:
    ...
    p.add_argument("--dry-run", action="store_true",
                   help="write the BDE + dump bridgeIQ auctions; skip "
    ...
    write_multi_deal_bde(deals, bde_path,
                         description=f"bridgeIQ diff batch, N={args.n}")
    print(f"[diff] Wrote {args.n} deals to {bde_path}")

    ours = [_drive_bridgeIQ(b, args.system) for b in deals]

    if args.dry_run:
        for i, (board, oauc) in enumerate(zip(deals, ours), 1):
            print(f"  bridgeIQ board {board.board_number:3d}: "
    ...
    if detected and detected != args.system:
        print(f"[diff] Q-Plus played {detected}; switching bridgeIQ to "
