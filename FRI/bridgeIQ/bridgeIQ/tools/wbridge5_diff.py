...
Workflow (until we find a wbridge5 CLI flag for TM-auto-connect):
  ...
  3. wbridge5 connects all four seats to our server. We run N
     deals, capture each auction, run bridgeIQ's bidder, and
...
# Allow `python tools/wbridge5_diff.py` from the bridgeIQ root.
...
from backend.pavlicek import (                    # noqa: E402
    number_to_deal,
)
from backend.wbridge5_driver import (             # noqa: E402
...
def _generate_deals(n: int, seed: int) -> List[BoardState]:
    ...
    """
    from backend.models import Card, Hand, Rank, Suit
...
def _drive_bridgeIQ(board: BoardState, system_name: str) -> List[Bid]:
    """Run bridgeIQ's native bidder on every seat in dealer order.
...
def main(argv=None) -> int:
    ...
    p.add_argument("--system", default="SAYC-wbridge5",
                   help="bridgeIQ system to test (default SAYC-wbridge5)")
    ...
    p.add_argument("--dry-run", action="store_true",
                   help="Skip the wbridge5 half — just run bridgeIQ's "
    ...
    # Dry-run path: just dump the bridgeIQ auctions to stdout and
    ...
    if args.dry_run:
        ...
        for d in deals:
            auc = _drive_bridgeIQ(d, args.system)
    ...
    for i, board in enumerate(deals, 1):
        ...
        try:
            ...
            theirs = []
        ours = _drive_bridgeIQ(board, args.system)
