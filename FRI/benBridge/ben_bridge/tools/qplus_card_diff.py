"""Q-Plus card-play diff: compare ben_bridge's card choices to Q-Plus's.

Pre-requisite: a BDL file with the same deals ben_bridge generates
...
For each deal:
  ...
  1. Build the same BoardState ben_bridge's bidder used.
  2. Set the contract from Q-Plus's BDL.
  3. For every card Q-Plus played, ask ben_bridge what it would
     play in that same position and tally matches / divergences.
     We always advance the position by Q-Plus's actual card
     (not ben_bridge's choice) so the comparison stays in sync.

ben_bridge's card engine: opening lead goes through BEN's NN
...
from ben_backend.bdl_reader import BDLDeal, load_bdl_file
...
def _replay_deal(engine: BridgeEngine, board: BoardState,
    ...
    """Diff ben_bridge's card play against Q-Plus's `qplus_play`
    ...
    # ben_bridge's engine sees a current legal hand.
...
def main(argv=None) -> int:
    ...
    p.add_argument("--n", type=int, default=5,
                   help="number of ben_bridge-generated deals to consider")
