...
SEAT_INDEX = {'N': 0, 'E': 1, 'S': 2, 'W': 3}
SEAT_LETTER = ['N', 'E', 'S', 'W']


class TableLobby:
    """Holds up to 4 websocket connections for one multi-human table.

    When all expected human seats have joined (or a 'start' signal comes from
    the first joiner), the driver is launched with a MultiSocketFactory that
    routes per-seat.
    """
    ...
    def __init__(self, table_id):
        ...
        self.sockets = {}  # abs_seat -> websocket
        self.expected_humans = set()  # set of abs_seat that must fill before start
        self.ready = asyncio.Event()
        self.finished = asyncio.Event()
        self.driver_task = None
        self.host_seat = None  # abs_seat of the client that configured the table
        self.params = {}  # host-supplied deal/board params

    @classmethod
    def get(cls, table_id):
        lobby = cls.tables.get(table_id)
        if lobby is None:
            lobby = TableLobby(table_id)
            cls.tables[table_id] = lobby
        return lobby

    @classmethod
    def drop(cls, table_id):
        cls.tables.pop(table_id, None)

    async def broadcast_status(self):
        status = {
            'message': 'lobby_status',
            'table': self.table_id,
            'expected': sorted(self.expected_humans),
            'connected': sorted(self.sockets.keys()),
        }
        for sock in list(self.sockets.values()):
            try:
                await sock.send(json.dumps(status))
            except Exception:
                pass

    def all_humans_connected(self):
        return self.expected_humans and self.expected_humans.issubset(set(self.sockets.keys()))


async def table_handler(websocket, table_id, seat, query_params, board_no, seed):
    """Handler for a websocket joining a multi-human table.

    This coroutine DOES NOT read from the websocket once the driver starts —
    the driver owns the socket and calls recv() itself. We just register,
    announce lobby state, then wait until the driver is done.
    """
    lobby = TableLobby.get(table_id)

    if seat in lobby.sockets:
        try:
            await websocket.send(json.dumps({
                'message': 'lobby_error',
                'reason': f'seat {SEAT_LETTER[seat]} already taken at table {table_id}'
            }))
        except Exception:
            pass
        return

    lobby.sockets[seat] = websocket

    # First client to arrive is the host: they carry the deal/seat setup params
    is_host = lobby.host_seat is None
    if is_host:
        lobby.host_seat = seat
        lobby.params = {k: v[0] for k, v in query_params.items() if v}
        # Seats that must be human. Flagged = caller passed N/E/S/W=x. If no
        # flag is passed, only the joining seat is treated as human; other
        # unfilled seats play as BEN bots.
        flagged = {SEAT_INDEX[ch] for ch in 'NESW' if query_params.get(ch, [None])[0]}
        if not flagged:
            flagged = {seat}
        lobby.expected_humans = flagged

    await lobby.broadcast_status()

    # Host kicks off the driver once every expected human seat is connected.
    if is_host:
        asyncio.create_task(_run_driver_when_ready(lobby, board_no, seed))

    # Hold the connection open until the driver is done OR the lobby is torn
    # down before the driver even started.
    try:
        await lobby.finished.wait()
    except (ConnectionClosedOK, ConnectionClosedError, ConnectionAbortedError):
        pass
    finally:
        lobby.sockets.pop(seat, None)
        if not lobby.sockets:
            TableLobby.drop(lobby.table_id)


async def _run_driver_when_ready(lobby, board_no, seed):
    # Poll until all expected seats are filled. The host set expected_humans.
    # If the host goes away before everyone arrives, abandon.
    poll_ms = 0
    while not lobby.all_humans_connected():
        ...
        if not lobby.sockets:
            ...
            lobby.finished.set()
            return
        # Timeout protection: give up after 5 minutes
        if poll_ms > 5 * 60 * 1000:
            for s in list(lobby.sockets.values()):
                try:
                    await s.send(json.dumps({'message': 'lobby_error', 'reason': 'timed out waiting for seats'}))
                except Exception:
                    pass
            lobby.finished.set()
            TableLobby.drop(lobby.table_id)
    ...
    params = lobby.params
    factory = human.MultiSocketFactory(lobby.sockets, verbose)
    driver = game.Driver(models, factory, Sample.from_conf(configuration, verbose), seed, dds, verbose)
    driver.human = [False, False, False, False]
    for ab in lobby.expected_humans:
        driver.human[ab] = True

    if params.get('H'):
        driver.human_declare = True
    if params.get('name'):
        driver.name = params['name']
    if params.get('R'):
        driver.rotate = True
    models.matchpoint = bool(params.get('M'))
    play_only = params.get('P') == '5'

    deal = params.get('deal')
    board_number = None
    bn_q = params.get('board_no')
    if bn_q and bn_q not in ('null', 'None'):
        board_number = int(bn_q)
    else:
    ...
    if deal:
        ...
        print(f"[Table {lobby.table_id}] Board: {board_number} {rdeal} {play_only}")
    else:
        if random:
            np.random.seed(board_number)
            rdeal = game.random_deal_board(board_number)
            print(f"[Table {lobby.table_id}] Board: {board_number} {rdeal}")
            driver.set_deal(board_number, *rdeal, False)
        else:
            rdeal = boards[board_no[0]]['deal']
            auction = boards[board_no[0]]['auction']
            print(f"[Table {lobby.table_id}] Board: {board_no[0]+1} {rdeal}")
    ...
    try:
        ...
        print(f'[Table {lobby.table_id}] Board played in {time.time() - t_start:0.1f} s')
        if not random and len(boards) > 0:
            board_no[0] = (board_no[0] + 1) % len(boards)
        gc.collect()
    except (ConnectionClosedOK, ConnectionClosedError, ConnectionAbortedError):
        print(f'[Table {lobby.table_id}] A client left.')
    except Exception as e:
        print(f'[Table {lobby.table_id}] Driver error: {e}')
        handle_exception(e)
    finally:
        lobby.finished.set()
        TableLobby.drop(lobby.table_id)
...
async def handler(websocket, board_no, seed):
    ...
    parsed_url_early = urlparse(path)
    query_params_early = parse_qs(parsed_url_early.query)

    # Multi-human table mode: activated only when ?table=... is present.
    # Without it, the classic single-socket flow runs unchanged.
    table_param = query_params_early.get('table', [None])[0]
    if table_param:
        seat_param = query_params_early.get('seat', [None])[0]
        if seat_param is None or seat_param.upper() not in SEAT_INDEX:
            await websocket.send(json.dumps({
                'message': 'lobby_error',
                'reason': 'missing or invalid seat= (expected N, E, S, or W)'
            }))
            return
        abs_seat = SEAT_INDEX[seat_param.upper()]
        await table_handler(websocket, table_param, abs_seat, query_params_early, board_no, seed)
        return
