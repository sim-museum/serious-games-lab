...
#     for each card played, init the x_play slice of the next trick. Pain in the ass
    ...
    async def play(self, contract, strain_i, decl_i, auction, opening_lead52, features):
        ...
        # Let MultiSocketFactory (if present) know which seat is declarer
        set_decl_seat = getattr(self.factory, 'set_declarer_seat', None)
        if set_decl_seat is not None:
            set_decl_seat(decl_i)
        for i in range(4):
            if self.human[i]:
                # We are declarer or human declare and dummy
                if decl_i == i or self.human_declare and decl_i == (i + 2) % 4:
                    card_players[3] = self.factory.create_human_cardplayer(self.models, 3, decl_hand, dummy_hand, contract, is_decl_vuln, abs_seat=i)
                    card_players[1] = self.factory.create_human_cardplayer(self.models, 1, dummy_hand, decl_hand, contract, is_decl_vuln, abs_seat=i)

                # We are lefty
                if i == (decl_i + 1) % 4:
                    card_players[0] = self.factory.create_human_cardplayer(self.models, 0, lefty_hand, dummy_hand, contract, is_decl_vuln, abs_seat=i)
                # We are righty
                if i == (decl_i + 3) % 4:
                    card_players[2] = self.factory.create_human_cardplayer(self.models, 2, righty_hand, dummy_hand, contract, is_decl_vuln, abs_seat=i)
    ...
    async def opening_lead(self, auction, aceking):
        ...
        if self.human[(decl_i + 1) % 4]:
            card_resp = await self.factory.create_human_leader(abs_seat=(decl_i + 1) % 4).async_lead()
            if self.verbose:
    ...
    async def bidding(self, bidding_only, bidding_only_auction):
        ...
        for i, level in enumerate(self.human):
            if level == 1:
                ...
                players.append(self.factory.create_human_bidder(vuln, hands_str[i], self.name, hint_bots[i], abs_seat=i))
                self.bot = None
            else:
